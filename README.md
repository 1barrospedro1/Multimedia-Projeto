# Multimedia-Projeto

* Aluno: Pedro Barros | Nº 33248
* Tema: Escape Room

O jogo é uma experiência de terror e escape room em primeira pessoa. O jogador encontra-se preso num pesadelo e precisa de explorar o ambiente, recolher ferramentas e resolver puzzles para destrancar a porta de saída antes que o tempo acabe. Se conseguir escapar da primeira sala, o jogador entra na fase final: uma cena de perseguição intensa onde terá de escapar e fugir do pesadelo.

## Funcionalidades
* **Controlador do Jogador:** Andamento, corrida, agachamento e salto fluídos utilizando o motor de física do Unity.
* **Visão com o rato:** Matemática de rotação de câmera customizada que ignora a interpolação de física para evitar micro-travamentos (micro-stutters).
* **Lanterna:** Uma lanterna que pode ser ativada ou desativada com um efeito 'flicker' personalizado baseado em corrotinas.
* **Movimento Dinâmico da Cabeça ('Head Bobbing'):** Movimento da câmera que simula naturalmente os passos de caminhada/corrida (e para quando o jogador está no ar).
* **Interação:** Mecânica para interagir com objetos.

## Controlos
* **W, A, S, D** - Mover
* **Mouse** - Olhar
* **Spacebar** - Saltar
* **Left Shift** - Correr
* **Ctrl** - Agachar
* **F** - Ligar/Desligar lanterna
* **E** - Interagir

## Desenvolvido Com
* Unity (6000.3.9f1)
* C#
* AudioMass

## Como Abrir o Projeto

1. Fazer o download ou clonar este repositório.
2. Abrir o Unity Hub, clicar em "Add" (ou Open) e selecionar a pasta do projeto.
3. Abrir com a versão do Unity 6000.3.9f1.
4. Na janela "Project", ir à pasta das Cenas e abrir a cena MainMenu.
5. Pressionar o botão "Play" no topo do editor para jogar

## Assets Utilizados
* **[VINTAGE LIVING ROOM 3D GAME PACK](https://assetstore.unity.com/packages/3d/environments/vintage-living-room-3d-game-pack-314464)**
* **[Basic Bedroom Starterpack](https://assetstore.unity.com/packages/3d/props/basic-bedroom-starterpack-215986)**
* **[Old Floor Linoleum FREE](https://assetstore.unity.com/packages/2d/textures-materials/floors/old-floor-linoleum-free-317315)**
* **[Game Input Controller Icons Free](https://assetstore.unity.com/packages/2d/gui/icons/game-input-controller-icons-free-285953)**
* **[Airduct BMT](https://assetstore.unity.com/packages/tools/level-design/airduct-bmt-227804)**
* **[Crowbar Free (Wrecking bar / Low-poly / 4k)](https://assetstore.unity.com/packages/3d/props/tools/crowbar-free-wrecking-bar-low-poly-4k-285620)**
* **[Padlock with password](https://assetstore.unity.com/packages/3d/props/interior/padlock-with-password-225067)**
* **[Free Pack - Cobwebs](https://assetstore.unity.com/packages/3d/props/free-pack-cobwebs-320977)**
* **[Rust Key](https://assetstore.unity.com/packages/3d/props/rust-key-167590)**
* **[Blood splatter decal package](https://assetstore.unity.com/packages/2d/textures-materials/blood-splatter-decal-package-7518)**
* **[Street Lamps 2](https://assetstore.unity.com/packages/3d/props/exterior/street-lamps-2-260395)**
* **[Mimic prototype](https://assetstore.unity.com/packages/3d/characters/creatures/mimic-prototype-245997)**

## Sons Utilizados

* Todos os sons/músicas utilizados(as) têm origem no site [Pixabay](https://pixabay.com/).

## Extras Utilizados

* [Char_ Fingery Frank_lp_01](https://sketchfab.com/3d-models/char--fingery-frank-lp-01-bd2497b495634fbc914b43f42312887c)
